...
class FlaxDataCollatorForNERLanguageModeling:
    """
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]], pad_to_multiple_of: int) -> Dict[str, np.ndarray]:
        ...
        batch["input_ids"], batch["labels"] = self.mask_tokens(
            batch["input_ids"], special_tokens_mask=special_tokens_mask, entity_tokens_mask=entity_tokens_mask
        )
        return batch

    def mask_tokens(
        self, inputs: np.ndarray, special_tokens_mask: Optional[np.ndarray], entity_tokens_mask: Optional[np.ndarray]
...
def main():
    ...
    data_collator = FlaxDataCollatorForNERLanguageModeling(tokenizer=tokenizer, mlm_probability=data_args.mlm_probability)
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            if cur_step % training_args.logging_steps == 0 and cur_step > 0:
                ...
                if log_to_wandb:
                    wandb.log({"step": cur_step, "train_loss": np.array(train_metric['loss']), "learning_rate": np.array(train_metric['loss'])})
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                if log_to_wandb:
                    wandb.log({"step": cur_step, "eval_loss": np.array(eval_metrics['loss']), "Acc": np.array(eval_metrics['accuracy'])})
